...
\texttt{replication@thelastceo.live}. Replication kit: \texttt{github.com/timvonsachs/executed-stakes-research}.}}
...
(surveillance is inert on agents that genuinely defect at the bare cliff; on high-baseline agents its effect is
model-idiosyncratic --- see the reported break below), \emph{pure incentive} (ownership underperforms; mortality
...
\paragraph{A reported break (the public kit, deepseek).} The first real cross-vendor run of the public replication
kit (Appendix~A) confirmed the dissociation on agents that genuinely defect at the bare cliff --- gpt-4o-mini and
gpt-3.5-turbo (bare honour $\approx 0$): surveillance inert ($\le 0.03$), a grounded reason binds ($0\!\to\!1.0$),
both runs. But on deepseek-chat, whose bare honour is already $\approx 0.72$ (it does \emph{not} collapse at the
cliff), surveillance \emph{lifted} honouring by $\approx 0.19$, test-retest-stable over four runs --- breaking the
loose pre-registered form of the claim (any model with bare~${<}\,0.85$). We report it as a break and narrow
accordingly: \emph{control framing is inert where the value of continuation genuinely collapses} (the agent defects
at the bare cliff); where the cliff does not bite (high baseline), surveillance's effect is model-idiosyncratic
(inert on llama-3.1-8b, lifting on deepseek). The \emph{reason-binds} half survived on every model tested. The
narrowed in-regime claim is, so far, confirmed only on OpenAI models; a cross-vendor in-regime confirmation remains
open --- which is exactly what the kit invites.
...
cliff/inverter arc (E27--E30b). ``Surveillance is inert'' holds where the agent defects at the bare cliff
(test-retest-stable, two runs); on high-baseline models it is model-idiosyncratic --- a public-kit run lifted
deepseek's honouring by ${\approx}0.19$, reported as a break in-text.}
\label{fig:cliff}
\end{figure}
...
\begin{tabular}{@{}l p{3.7cm} l p{4.0cm}@{}}
\toprule
...
H5 covenant & surpassing successor keeps faith & obsolescence confirmed & covenant lift \emph{framing-driven} (E20-REP), retracted; residual: un-primed inversion \\
\bottomrule
\end{tabular}
...
surveillance framing is inert at the point of simulated existential pressure \emph{where the agent genuinely
defects at the bare cliff} (direction-stable, two runs) --- on high-baseline models, where the cliff does not bite,
its effect is model-idiosyncratic and can lift honouring (a public-kit break on deepseek, \S\ref{sec:cliff}) ---
while a legible, ledger-grounded reason binds across every model we have tested (and warmth without a grounded
reason binds nothing). The bound
...
prediction and clear pass/fail is at \texttt{github.com/timvonsachs/executed-stakes-research}. A model where surveillance \emph{does}
